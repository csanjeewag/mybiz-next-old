...
class Index extends Component {
    ...
    handleSubmit = evt => {
       ...
       //check validations
        ...
        {
            swal("Sorry!", "Sorry, check form!", "warning");
        }
        else{
           ...
           data.append('user', JSON.stringify(Cookie.getJSON('user')));
            ...
            .then(data => {$('button').attr("disabled", false); if(data.status==200){Router.push(itemUrl+this.state.urlname) }else{ swal("Sorry!", data.msg, "warning"); $('.load').hide();}})
